					 ...
					 const char __user *ubuf, size_t count, loff_t *offset)
{
	...
	*comma1 = *comma2 = '\0';
	...
	if (attr == 0)
		...
		dma_attrs = DMA_ATTR_SYS_CACHE_NWA;
	else
		goto invalid_format;
					   ...
					   void *ignored)
{
	...
	/* Debug: Entry */
	pr_err("%s: Entry: dev=%s, domain=%p\n", __func__, dev_name(dev), domain);

	/* Make sure we can allocate and use a buffer */
	ps_printf(dev_name(dev), s, "Allocating coherent buffer");

	pr_err("%s: Attempting dma_alloc_coherent. size=%zu\n", __func__, size);
	data = dma_alloc_coherent(dev, size, &iova, GFP_KERNEL);

	if (!data) {
		pr_err("%s: dma_alloc_coherent FAILED to allocate memory\n", __func__);
		ret = -ENOMEM;
	} else {
		int i;

		/* Debug: Allocation success details */
		pr_err("%s: dma_alloc_coherent succeeded. vaddr=%p, iova=%pad\n",
		       __func__, data, &iova);

		ps_printf(dev_name(dev), s, "  -> SUCCEEDED\n");
		ps_printf(dev_name(dev), s, "Using coherent buffer");

		pr_err("%s: Starting verification loop (742 iterations)\n", __func__);
		...
		for (i = 0; i < 742; ++i) {
			...
			/* Note: This memset happens on every iteration */
			...
			*p = val;
			...
			if ((*p) != val + 1) {
				...
				pr_err("%s: Verification FAILED at iter=%d (offset=%d). Expected=%d, Read=%d\n",
				       __func__, i, ind, val + 1, *p);
		...
		if (!ret) {
			pr_err("%s: Verification loop completed successfully\n", __func__);
			ps_printf(dev_name(dev), s, "  -> SUCCEEDED\n");
		}

		pr_err("%s: Freeing coherent buffer. vaddr=%p, iova=%pad\n",
		       __func__, data, &iova);
					   ...
					   void *ignored)
{
	...
	u8 *data = NULL;
	dma_addr_t iova;
	phys_addr_t phys_addr; /* Added variable to fix compilation error */

	/* Debug: Entry */
	pr_err("%s: Entry: dev=%s\n", __func__, dev_name(dev));

	ps_printf(dev_name(dev), s, "Basic DMA API test");

	pr_err("%s: Starting loop (1000 iterations), size=%zu\n", __func__, size);

	/* Make sure we can allocate and use a buffer */
	for (i = 0; i < 1000; ++i) {
		/* Debug: Log progress every 100 iterations */
		if (i % 100 == 0)
			pr_err("%s: Iteration %d/1000\n", __func__, i);

		data = kmalloc(size, GFP_KERNEL);
		if (!data) {
			pr_err("%s: Iter %d: kmalloc failed\n", __func__, i);
			ret = -ENOMEM;
			goto out;
		}
		...
		if (ret) {
			pr_err("%s: Iter %d: Mapping check FAILED. ret=%d, iova=%pad, phys=%pa\n",
			       __func__, i, ret, &iova, &phys_addr);
			goto out;
		}
		...
		for (j = 0; j < size; ++j) {
			if (data[j] != 0xa5) {
				pr_err("%s: Iter %d: Data mismatch at offset %d. Expected 0xa5, got %02x\n",
				       __func__, i, j, data[j]);
...
out:
	/* Debug: cleanup leaked data if we jumped out on error */
	if (data) {
		pr_err("%s: Cleaning up leaked data buffer on error path\n", __func__);
		kfree(data);
	}
						   ...
						   void *ignored)
{
	...
	/* Debug: Entry log with pointer values */
	pr_err("%s: Entry: ddev=%p, s=%p\n", __func__, ddev, s);

	mutex_lock(&ddev->state_lock);
	pr_err("%s: Acquired state_lock\n", __func__);

	if (!iommu_debug_usecase_reset(ddev)) {
		pr_err("%s: Failed to reset usecase\n", __func__);
		goto out;
	}
	dev = ddev->test_dev;
	pr_err("%s: Use case reset successful, test_dev=%p\n", __func__, dev);

	/* Alloc Test */
	pr_err("%s: Starting __functional_dma_api_alloc_test\n", __func__);
	ret = __functional_dma_api_alloc_test(dev, s, ddev->domain, sizes);
	pr_err("%s: __functional_dma_api_alloc_test completed. ret=%d\n", __func__, ret);

	/* Basic Test */
	pr_err("%s: Starting __functional_dma_api_basic_test\n", __func__);
	ret |= __functional_dma_api_basic_test(dev, s, ddev->domain, sizes);
	pr_err("%s: __functional_dma_api_basic_test completed. Accumulated ret=%d\n", __func__, ret);

	/* Map SG Test */
	pr_err("%s: Starting __functional_dma_api_map_sg_test\n", __func__);
	ret |= __functional_dma_api_map_sg_test(dev, s, ddev->domain, sizes);
	pr_err("%s: __functional_dma_api_map_sg_test completed. Accumulated ret=%d\n", __func__, ret);

out:
	mutex_unlock(&ddev->state_lock);
	pr_err("%s: Released state_lock\n", __func__);

	if (ret) {
		pr_err("%s: Test FAILED with ret=%d\n", __func__, ret);
		seq_printf(s, "FAIL %d\n", ret);
	} else {
		pr_err("%s: Test SUCCEEDED\n", __func__);
		seq_puts(s, "SUCCESS\n");
	}
